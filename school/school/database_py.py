...
class Main_DB:
    ...
    def add_student(self, std_id, std_name, phone, class_id, class_num, school_id, pdf_name=None, high=False):
        cursor = self.database.cursor()
        sql = f"SELECT * FROM students WHERE student_id ='{std_id}' AND school_id='{str(school_id)}' LIMIT 1"
        cursor.execute(sql)
        result = cursor.fetchall()
        if result:
            ...
            if high:
                ...
                sql = f"INSERT INTO students (student_id, name, phone, class_id, class_num, certificate, school_id) " \
                  f"VALUES ('{std_id}', '{std_name}', '{phone}', '{class_id}', '{class_num}', '{pdf_name}','{school_id}')"
            cursor.execute(sql)
            self.database.commit()
    ...
    @staticmethod
    def download_wait(directory, timeout, number_file=None):
        seconds = 0
        dl_wait = True
