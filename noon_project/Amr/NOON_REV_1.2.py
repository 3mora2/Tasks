...
class SouCode:
    ...
    PRICE_TEXT_SELECTOR = (By.CSS_SELECTOR, 'span.sellingPrice span.value')
    SELLER_TEXT_SELECTOR = (By.CSS_SELECTOR, 'p.sellerName a')
    EXPRESS_SELECTOR = (By.CSS_SELECTOR, 'div.bottomRow div.container img')
    RATE_1_SELECTOR = (By.CSS_SELECTOR, 'div.detail_column_third > div.detail_row:nth-child(1) > div')
    RATE_2_SELECTOR = (By.CSS_SELECTOR, 'div.detail_column_third > div.detail_row:nth-child(2) > div')
    # driver.execute_script("return parseFloat(document.querySelector('.priceNow').textContent.replace('EGP',''));")
    # SELLER_TEXT_SELECTOR = (By.CSS_SELECTOR, 'a.storeLink')
    # EXPRESS_SELECTOR = (By.CSS_SELECTOR, 'div.estimator_right > div > img')
    # driver.find_elements_by_css_selector('div.detail_percentage')[1].text
    # driver.find_elements_by_css_selector('.allOffers')
    # driver.find_elements_by_css_selector('div.regular > strong')[1].text
    # driver.find_elements_by_css_selector('.soldby > strong')[1].text.split(' ')[-1]
    # driver.find_elements_by_css_selector('div[direction="row"] > div > img')[1].get_attribute('alt')
    ...
    def Search(self):
        ...
        for i in range(2, end_cell + 1):
            ...
            try:
                ...
                sleep(2)
                price = float(WebDriverWait(self.driver, 3).until(
                    (ec.visibility_of_element_located(self.PRICE_TEXT_SELECTOR))).text)
                ...
                if seller in group1:
                    ...
                    try:
                        rat1 = float(WebDriverWait(self.driver, 2).until(
                            (ec.visibility_of_element_located(self.RATE_1_SELECTOR))).text.replace('%', ''))
                        rat2 = float(WebDriverWait(self.driver, 2).until(
                            (ec.visibility_of_element_located(self.RATE_2_SELECTOR))).text.replace('%', ''))
                        print(rat1 + rat2)
                        ...
                        avn = avrnd
                        print(rat1, rat2)
                Offer_number = len(self.driver.find_elements_by_css_selector('ul.offersList > li'))
                        ...
                        try:
                            for ELEMENT in offer:
                                ...
                                if 'selectedItem' not in ELEMENT.get_attribute('class'):
                                    best.append((float(ELEMENT.find_element_by_css_selector(
                                        'span.sellingPrice > span > span.value').text),
                                                 ELEMENT.find_element_by_css_selector('div.soldBy').text.split('\n')[0],
                                                 ELEMENT.find_element_by_css_selector('.container img').get_attribute(
                                                     'alt')))
