...
class NOON:
    ...
    # SUBMIT_SELECTOR = (
    By.XPATH, '//div[calss="btnWrapper"]/div[text()="Submit"] | //div[contains(@class,"showAlert")]//div[contains(@class,"solid")][2]')
    #############################################
    ...
    def add_qu(self, q):
        ...
        try:
            ...
            try:
                ...
                sleep(1)
                WebDriverWait(self.driver, 6).until((ec.visibility_of_all_elements_located(self.QUANTITY_SITE_CHOSE_SELECTOR)))[0].click()
                sleep(1)
    ...
    def search_bar(self, code, url, add=False):
        ...
        self.driver.get(url)
        sleep(1)
        WebDriverWait(self.driver, 5).until((ec.visibility_of_element_located(self.SEARCH_INPUT_SELECTOR))).send_keys(code, Keys.ENTER)
        sleep(3)
        if add:
            print('- add')
            ...
            try:
                for i in range(3):
                    sleep(1)
                    if code in WebDriverWait(self.driver, 5).until(ec.visibility_of_element_located((By.CSS_SELECTOR, 'tbody > tr:nth-child(1) > td:nth-child(5)'))).text:
                        print('- found')
                        WebDriverWait(self.driver, 5).until(ec.visibility_of_element_located((By.CSS_SELECTOR, 'tbody > tr:nth-child(1) > td > a'))).click()
                        return True
                print('- Not found')
                return False
            except:
    ...
    def change_quantity(self, i):
        ...
        try:
            ...
            try:
                ...
                WebDriverWait(self.driver, 6).until((ec.visibility_of_all_elements_located(self.QUANTITY_SITE_CHOSE_SELECTOR)))[0].click()
                sleep(1)
                WebDriverWait(self.driver, 6).until((ec.visibility_of_element_located(self.QUANTITY_INPUT_SELECTOR))).send_keys(str(content_new))
                sleep(1)
            except:
    ...
    def Search(self):
        ...
        for i in range(2, end_cell + 1):
            ...
            seller = WebDriverWait(self.driver, 3).until((ec.visibility_of_element_located(self.SELLER_TEXT_SELECTOR))).text
            try:
                express = WebDriverWait(self.driver, 3).until((ec.visibility_of_element_located(self.EXPRESS_SELECTOR))).get_attribute('alt')
