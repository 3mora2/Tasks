...
class SouCode:
    ...
    def Get_URL(self):
        ...
        if all_type is False:
            self.driver.find_element_by_css_selector('div.header').click()
            sleep(2)
            self.driver.find_element_by_css_selector('div > div.active > div:nth-child(5) > div').click()
            sleep(5)
        q = 0
        while True:
            for element in self.driver.find_elements_by_css_selector('table > tbody > tr'):
                ...
                if not all_type:
                    un = element.find_element_by_css_selector('td:nth-child(3)').text.strip()
                    if un in l:
                        print('- Found')
                        continue
