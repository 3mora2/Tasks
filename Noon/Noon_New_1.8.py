...
class NOON:
    ...
    def Search(self):
        ...
        for i in range(2, self.sheet.max_row+1):
            ...
            try:
                data = json.loads(self.driver.find_element_by_css_selector('#__NEXT_DATA__').get_attribute('innerText'))
                if not data['props']['pageProps']['catalog']['product']['variants'][0]['offers']:
                    continue

                express = data['props']['pageProps']['catalog']['product']['variants'][0]['offers'][0].get('is_fbn')
                express_static = True if express == 1 else False
                price = data['props']['pageProps']['catalog']['product']['variants'][0]['offers'][0].get('sale_price')
                price = price if price else \
                    data['props']['pageProps']['catalog']['product']['variants'][0]['offers'][0]['price']
                price = float(price)
                seller = data['props']['pageProps']['catalog']['product']['variants'][0]['offers'][0]['store_name']
                self.sheet[f'D{i}'] = seller
                brand = data['props']['pageProps']['catalog']['product'].get('brand')
                self.sheet[f'J{i}'] = brand
                prices = [float(p.get('sale_price')) if p.get('sale_price') else p.get('price') for p in
                          data['props']['pageProps']['catalog']['product']['variants'][0]['offers']]
                prices.sort()

                Offer_number = prices.__len__() - 1
                try:
                    if brand in brand_exception:
                        ...
                        self.sheet[f'F{i}'] = express_static
                    ...
                    pass
                dict_seller = dict()
                for element in data['props']['pageProps']['catalog']['product']['variants'][0]['offers']:
                    dict_seller[element['store_name']] = element.get('is_fbn')

                if main_seller.strip() in dict_seller.keys():
                    if dict_seller[main_seller.strip()] == 1:
                        your_state = True
                    else:
                        your_state = False
                    self.sheet[f'AA{i}'].value = your_state
                else:
                    print('- You not found')
                    self.sheet[f'AA{i}'].value = 'You not found'
                    add_new = True
                    your_state = False
                ...
                if main_seller in seller:
                    if Offer_number == 0:
                        ...
                        yearn = self.price_eran(price_earn=price, your_price_earn=your_price,
                                                express_earn=your_state)
                        self.sheet[f'G{i}'] = yearn
                        if yearn < 0:
                            new_price = self.price_count(your_price, your_state, earn)
                            self.sheet[f'L{i}'].value = new_price
                        print(f'- You are Main, Earn = {yearn}')
                    else:
                        min_pr = self.price_count(your_price, your_state, earn)
                        seller_price = prices[1]
                ...
                print(e)
                seller = price = express_static = new_price = None
            ...
            try:
                self.sheet[f'AB{i}'] = main_seller in seller
            except:
                pass
...
if __name__ == "__main__":
    ...
    if not debug:
        ...
        while True:
            self = NOON(file_name=file)
            self.Search()
    else:
        file = 'assad.xlsx'
        main_seller = 'Beauty & Health'
        email = 'bh4708251@gmail.com'
        password = 'Assad203064'
        gaverage = float(0.1)
        content_inp = int(1)
        content_new = int(5)
        earn_all = float(25)
