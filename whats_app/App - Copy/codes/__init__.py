...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def Handel_Button(self):
        ...
        self.pushButton_51.clicked.connect(self.Open_File_AddMember)
...
"}")
        ...
        self.pushButton_3.setText(QCoreApplication.translate("Dialog", u"\u0627\u063a\u0644\u0627\u0642", None))
        self.pushButton_2.setText(QCoreApplication.translate("Dialog", u"\u0627\u0636\u0627\u0641\u0629", None))
        ...
        self.pushButton_3.clicked.connect(self.close_dialog)
        self.tableWidget.verticalHeader().setVisible(False)
        self.tableWidget.horizontalHeader().setStretchLastSection(True)
        self.tableWidget.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
                    ...
                    if num.strip() != '' and num.startswith('+'):
                        ...
                        if result:
                            ...
                            if really not in old_number:
                                ...
                                self.tableWidget.setItem(r, 0, __q_tablewidget_item)
                                self.tableWidget.setItem(r, 1, QTableWidgetItem(str(country)))
                                self.tableWidget.setItem(r, 2, QTableWidgetItem(str(openfile)))

    def add_to_table(self):
        if self.sender_dialog == 'pushButton_51':
            tableWidget = self.tableWidget_12
        else:
            return

        table_codes = [self.tableWidget.item(i, 0).text() for i in range(self.tableWidget.rowCount()) if self.tableWidget.item(i, 0).checkState() == Qt.CheckState.Checked]
        file_name = set(self.tableWidget.item(i, 2).text() for i in range(self.tableWidget.rowCount()) if self.tableWidget.item(i, 0).checkState() == Qt.CheckState.Checked)
