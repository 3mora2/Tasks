...
try:
    while page < 49:#
        ...
        for urll in url:
            ...
            ht=respons.html
            try:title=ht.find('.rh_page__title',first=True).text
            except:title=None

            try:Type=ht.find('.property-breadcrumbs',first=True).xpath('//li')[-1].text.split(':')[1].strip()
            except:Type=None

            try:pic=ht.xpath('//*[@id="property-featured-image"]/a',first=True).attrs['href']
            except:
                pic=''
                try:
            ...
            try:price=ht.find('.price',first=True).text
            except:price=None

            try:ref=ht.find('.id',first=True).text
            except:ref=None

            try:bedrooms=ht.find('.prop_bedrooms',first=True).find('.figure',first=True).text
            except:bedrooms=None

            try:bathrooms=ht.find('.prop_bathrooms',first=True).find('.figure',first=True).text
            except:bathrooms=None

            try:area=ht.find('.prop_area',first=True).xpath('//div/div')[0].text
            except:area=None
