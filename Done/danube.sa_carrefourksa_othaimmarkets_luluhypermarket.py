...
class LuluhyperMarket:
    ...
    def start(self, url, s):
        ...
        for num, link in enumerate(list_u):
            ...
            try:
                brand = self.driver.find_element_by_css_selector('.pdp-info-main > strong').text.replace('Brand',
                                                                                                         '').replace(
                    ':', '').strip()
            except:
                brand = None
            try:
                ...
                cat = None
            try:
                old_price = self.driver.find_element_by_css_selector('.prod-price strike').text
            except:
                old_price = None
            try:
